...
with patch.dict('sys.modules', {
    ...
    with patch('clasificador.TinyDB'), \
         patch('clasificador.faiss'), \
         patch('clasificador.FaceAnalysis'):
        import clasificador
...
class TestExtractAllFaceEmbeddings:
    @patch('clasificador.cv2.imread')
    @patch('clasificador.app.get')
    def test_extract_all_face_embeddings_multiple_faces(self, mock_app_get, mock_imread):
...
class TestSaveUnknownFace:
    ...
    def test_save_unknown_face_success(self, mock_np_save, mock_write_index, 
                                     mock_index_add, mock_db_insert,
                                     mock_generate_thumbnail, mock_extract_embedding):
...
class TestIdentifyFace:
    ...
    def test_identify_face_found(self, mock_db_get, mock_index_search, mock_extract_embedding):
...
class TestGetUnclassifiedFaces:
    ...
    def test_get_unclassified_faces(self, mock_db_all):
        ...
        mock_faces = [
            ...
            {'face_id': '2', 'name': 'John', 'event_id': 'evt2'},  # Has name, should be filtered out
