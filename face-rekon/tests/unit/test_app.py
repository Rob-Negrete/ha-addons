...
class TestRecognizeEndpoint:
    ...
    @patch("os.makedirs")
    @patch("os.remove")
    def test_recognize_faces_success(
        self, mock_remove, mock_makedirs, mock_save_unknown, mock_identify
    ):
    ...
    @patch("os.makedirs")
    @patch("os.remove")
    def test_recognize_faces_unknown(
        self, mock_remove, mock_makedirs, mock_save_unknown, mock_identify
    ):
    ...
    def test_recognize_data_uri_format(self):
        ...
        request_data = {"image_base64": data_uri_image, "event_id": "test_event"}

        with patch("clasificador.identify_all_faces") as mock_identify, patch(
            "os.makedirs"
        ), patch("os.remove"):
            mock_identify.return_value = []
    ...
    @patch("os.makedirs")
    @patch("os.remove")
    def test_recognize_exception_handling(
        self, mock_remove, mock_makedirs, mock_identify
    ):
