...
Tests real TinyDB operations with actual ML models for true integration testing.
"""
import os
...
class TestDatabaseIntegration:
    ...
    def test_database_initialization(self, clean_test_env, shared_ml_models):
        """
        Integration test: Verify TinyDB database and FAISS index initialization
        """
        # Use the real models from session-scoped fixture
        clasificador = shared_ml_models["clasificador"]

        # Verify database instance exists and is functional
        assert clasificador.db is not None
        assert hasattr(clasificador.db, "insert")
        assert hasattr(clasificador.db, "search")

        # Verify FAISS index exists and has correct dimensions
        assert hasattr(clasificador, "index")
        if clasificador.index is not None:
            # FAISS index should be 512-dimensional for InsightFace embeddings
            assert clasificador.index.d == 512
